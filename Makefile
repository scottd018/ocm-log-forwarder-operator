...
IMG ?= "ghcr.io/scottd018/ocm-log-forwarder-operator:latest"
...
test-e2e: ## Run E2E tests against a running Kubernetes cluster.
	go test github.com/scottd018/ocm-log-forwarder-operator/test/e2e -tags=e2e_test -count=1
...
ifeq (,$(shell which opm 2>/dev/null))
	...
	curl -sSLo $(OPM) https://github.com/operator-framework/operator-registry/releases/download/dustinscott.io/$${OS}-$${ARCH}-opm ;\
	chmod +x $(OPM) ;\
	}
else
OPM = $(shell which opm)
...
# dustinscott.io/ocm-log-forwarder-operator-bundle:$VERSION and dustinscott.io/ocm-log-forwarder-operator-catalog:$VERSION.
IMAGE_TAG_BASE ?= dustinscott.io/ocm-log-forwarder-operator
...
build-cli: ## Build the companion CLI.
	go build -o bin/ocmlogctl cmd/ocmlogctl/main.go
