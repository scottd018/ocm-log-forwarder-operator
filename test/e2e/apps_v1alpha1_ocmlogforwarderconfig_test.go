...
//
// appsv1alpha1OCMLogForwarderConfig tests
//
func appsv1alpha1OCMLogForwarderConfigChildrenFuncs(tester *E2ETest) error {
	// TODO: need to run r.GetResources(request) on the reconciler to get the mutated resources
	if len(ocmlogforwarderconfig.CreateFuncs) == 0 {
		return nil
	}

	workload, err := ocmlogforwarderconfig.ConvertWorkload(tester.workload)
	if err != nil {
		return fmt.Errorf("error in workload conversion; %w", err)
	}

	resourceObjects, err := ocmlogforwarderconfig.Generate(*workload, nil, nil)
	if err != nil {
...
func appsv1alpha1OCMLogForwarderConfigNewHarness(namespace string) *E2ETest {
	return &E2ETest{
		namespace:          namespace,
		unstructured:       &unstructured.Unstructured{},
		workload:           &appsv1alpha1.OCMLogForwarderConfig{},
		sampleManifestFile: "../../config/samples/apps_v1alpha1_ocmlogforwarderconfig.yaml",
		getChildrenFunc:    appsv1alpha1OCMLogForwarderConfigChildrenFuncs,
		logSyntax:          "controllers.apps.OCMLogForwarderConfig",
	}
}

func (tester *E2ETest) appsv1alpha1OCMLogForwarderConfigTest(testSuite *E2ECollectionTestSuite) {
...
func (testSuite *E2ECollectionTestSuite) Test_appsv1alpha1OCMLogForwarderConfig() {
	tester := appsv1alpha1OCMLogForwarderConfigNewHarness("")
	tester.appsv1alpha1OCMLogForwarderConfigTest(testSuite)
}
